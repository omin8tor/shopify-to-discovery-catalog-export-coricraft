...
PRODUCT_METAFIELD_MAPPINGS = [
    ...
    ["svm.custom.lead_time", "svm.custom.lead_time", 1, lambda x: (x.replace(":", "").strip() if isinstance(x, str) else ", ".join(i.replace(":", "").strip() for i in (json.loads(x) if isinstance(x, str) else x)))],
