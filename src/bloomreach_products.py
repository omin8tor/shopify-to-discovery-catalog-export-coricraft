...
def create_product(product, shopify_url):
    ...
    # container for the transformed product attributes
    out_pa = out_product["attributes"]

    out_pa["url"] = f"https://{shopify_url}/products/" + in_pa["sp.handle"]

    # Process labels from two possible keys
    labels_list = []
    ...
    for v_id, variant in product["variants"].items():
        ...
        if "sv.selectedOptions" in in_va and in_va["sv.selectedOptions"]:
            for option in in_va["sv.selectedOptions"]:
                if option.get("name") and "Color" in option["name"]:
                    out_va["color"] = option["value"]
                if option.get("name") and "Size" in option["name"]:
        ...
        if "sv.availableForSale" in in_va and in_va["sv.availableForSale"]:
            ...
            out_va["availability"] = True  # ← You might want this to be False instead
            out_va["sv.availableForSale"] = False

        # Set variant thumb image
